...
func TestResponseSizeBytes_OutcomeFailure(t *testing.T) {
	reader := sdkmetric.NewManualReader()
	provider := sdkmetric.NewMeterProvider(sdkmetric.WithReader(reader))
	...
	m := findMetric(t, collectMetrics(t, reader), "response_size_bytes")
...
func TestResponseSizeBytes_WithFilter(t *testing.T) {
	reader := sdkmetric.NewManualReader()
	provider := sdkmetric.NewMeterProvider(sdkmetric.WithReader(reader))
	...
	for _, sm := range rm.ScopeMetrics {
		for _, m := range sm.Metrics {
			if m.Name == "response_size_bytes" {
				t.Fatalf("expected no response_size_bytes datapoint for filtered request")
			}
		}
	}
}
